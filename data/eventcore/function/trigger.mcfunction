...
execute unless data storage eventcore:sys config_call{version_check:0b} unless data storage eventcore:config {version:"2.2.0"} run tellraw @a [{"text":"[EventCore] ","color":"red","bold":true},{"text":"Sürüm uyumsuzluğu (beklenen: 2.2.0). /reload yapın.","color":"white"}]
execute unless data storage eventcore:sys config_call{version_check:0b} unless data storage eventcore:config {version:"2.2.0"} run return fail

# Geciktirme — config:{delay:N} varsa defer et, sonraki satırları atla
