...
execute unless data storage eventcore:sys args.type unless data storage eventcore:sys event_queue[0] unless data storage eventcore:sys config_call{silent:1b} if data storage eventcore:config errors{show:1b} run tellraw @a [{"storage":"eventcore:config","nbt":"messages.err_prefix","color":"red","bold":true},{"text":" ","color":"white"},{"storage":"eventcore:config","nbt":"messages.err_empty","color":"white"}]
